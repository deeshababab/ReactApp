...
export default function HomeScreen(props) {
  ...
  const checkcat = (v, data) => {
    ...
    data.map((item, i) => {
      if (
        ...
        setList1(arr);
        FieldSet(item.sub_categories[0].category.id);
        setcheck(true);
      }
    });
  ...
  const FieldSet = v => {
    ...
    fetch(`http://www.amacoerp.com/amaco/public/api/columns/${v}`)
      .then(result => result.json())
      .then(data => {
        if (data[0].column.length) setList3(data[0].column);
      });
  };
    ...
    return (
      ...
      >
        ...
        >
          {category.map((item, i) => (
            <Menu.Item
              onPress={() => checkcat(item.category.id, List1)}
  ...
  return (
    ...
    <>
      ...
      <View style={styles.container}>
        <Provider>
          ...
          >
            ...
            >
              {List.map((item, i) => (
                <Menu.Item
                  onPress={() => checkcat(item.category.id, List)}
                  title={item.category.name}
                />
              ))}
            </Menu>
        ...
        {!check && (
          <Button onPress={handleFormSubmit} style={{ padding: 20 }}>
            submit
          </Button>
        )}
      </View>
